...
/*    public float getDPI() {
    ...
    public void getDefaultData() {
        Log.d("aaaaaaaaa", "getDefaultData:  " + new MyApplication().getDeviceID().trim());
        //String urlString = Url.selectDefaultDataUrl;
        String urlString = Url.getscidBinfo;
        OkHttpUtils.post()
                .url(urlString)
                .addParams("deviceId", new MyApplication().getDeviceID().trim())
                .build()
                .execute(new StringCallback() {
                    ...
                    public void onResponse(String response, int id) {
                        Log.i("456789", response);
                        if (response != null && !TextUtils.isEmpty(response)) {
                            Log.i("4wxa", response);
                            try {
                                DefaultDataBean defaultDataStrList = MyJsonUtil.getDefaultBeanForJson(response, DefaultDataBean.class);
                                Log.i("4wxac1", "onResponse: " + defaultDataStrList);
                                Log.i("4wxac1", "onResponse3: " + defaultDataStrList.getBody());
                                Log.i("4wxac1", "onResponse2: " + defaultDataStrList.getBody().getLzUsescid());
                                Log.i("4wxac1", "onResponse1: " + defaultDataStrList.getBody().getLzUsescid().getNearbyPeopleNum());
                                if (defaultDataStrList != null) {
                                    Log.i("4wxac", response);
                                    MyToast.show(getApplicationContext(), "数据获取成功", isDeBug);
                                    PrefUtils.putInt(getApplicationContext(), Constant.wxFunction[0], defaultDataStrList.getBody().getLzUsescid().getNearbyPeopleNum());
                                    PrefUtils.putString(getApplicationContext(), Constant.wxFunction[1], defaultDataStrList.getBody().getLzUsescid().getSendMsgContent());
                                    PrefUtils.putInt(getApplicationContext(), Constant.wxFunction[2], defaultDataStrList.getBody().getLzUsescid().getAddPeopleNum());
                                    PrefUtils.putInt(getApplicationContext(), Constant.wxFunction[3], defaultDataStrList.getBody().getLzUsescid().getGroups());
                                    PrefUtils.putInt(getApplicationContext(), Constant.wxFunction[4], defaultDataStrList.getBody().getLzUsescid().getAccounts());
                                    PrefUtils.putInt(getApplicationContext(), Constant.wxFunction[5], defaultDataStrList.getBody().getLzUsescid().getPushAccounts());
                                    PrefUtils.putInt(getApplicationContext(), Constant.wxFunction[6], defaultDataStrList.getBody().getLzUsescid().getPushfriends());
                                    PrefUtils.putInt(getApplicationContext(), Constant.wxFunction[7], defaultDataStrList.getBody().getLzUsescid().getDriftBottleNum());
                                    PrefUtils.putString(getApplicationContext(), Constant.wxFunction[8], defaultDataStrList.getBody().getLzUsescid().getDriftBottleContent());
