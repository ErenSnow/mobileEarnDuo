...
public class MyJsonUtil {
     ...
     * DefaultBean专用
     */
    public static <T>  T  getDefaultBeanForJson(String response,Class<T> c){
        Gson gson = new Gson();
        DefaultDataBean defaultDataBean = new DefaultDataBean();
        String jsonBodyStr = null;
        T t = null;
        if ((response != null && !TextUtils.isEmpty(response) && !response.equals("[]"))){
            Log.d("Dataa", "数据为：" + response);
            defaultDataBean = gson.fromJson(response,DefaultDataBean.class);
            Log.d("Data",defaultDataBean.getBody().toString());
            jsonBodyStr = gson.toJson(defaultDataBean);
        }
        Log.d("Data", "数据为：" + jsonBodyStr);
        if (defaultDataBean.isSuccess()&&jsonBodyStr != null && !TextUtils.isEmpty(jsonBodyStr) && !jsonBodyStr.equals("[]")&&!"".equals(jsonBodyStr)) {
            t =  (T)gson.fromJson(jsonBodyStr,c);
        }else {
            Log.e("err",defaultDataBean.getMsg()+":"+defaultDataBean.getErrorCode());
        }
        return t;
    }
